...
def load_data(file_path):
    """
    Load fresh data from CSV every call.
    Falls back to synthetic data when the file is not found.

    Handles two CSV layouts:
      A) Single 'Time' column that already contains a full datetime string.
      B) Separate 'Date' and 'Time' columns — combines them into one
         'Time' column (datetime) so the rest of the app works unchanged.
    Uses format='mixed' to avoid the dateutil UserWarning in all cases.
    """
    try:
        df = pd.read_csv(file_path)

        if 'Date' in df.columns and 'Time' in df.columns:
            # Layout B: combine Date + Time into a single datetime column
            ...
            # Layout A: Time column already holds full datetime strings
...
def _new_live_row(base_row: pd.Series) -> pd.Series:
    """Generate one new live reading by walking the last known values with a small random drift."""
    row = base_row.copy()
    drifts = {
...
def get_live_df(base_df: pd.DataFrame) -> pd.DataFrame:
    ...
    Every rerun unconditionally appends one synthetic tick (random drift
    from the last row) so KPIs and charts always receive a new data point.
    If the source CSV has genuinely new rows since the last check, those
    are ingested instead so real sensor data always takes priority.

    Root-cause of the old bug: the previous version compared the buffer's
    last timestamp (which drifted into the future via synthetic ticks) to
    the CSV's last timestamp. This made the "new CSV rows" branch always
    evaluate to True, but base_df[base_df["Time"] > buf_last_time]
    returned zero rows because the buffer was already ahead of the CSV,
    silently killing all updates after the very first rerun.
    Fixed by tracking csv_last_seen separately in session_state so the
    two clocks never interfere.
    """
    # Initialise buffer + CSV cursor on very first run
    ...
    # Ingest genuinely new CSV rows if any have arrived
    if "Time" in base_df.columns and csv_last_seen is not None:
        new_rows = base_df[base_df["Time"] > csv_last_seen]
        if len(new_rows) > 0:
            buf = pd.concat([buf, new_rows], ignore_index=True)
            st.session_state["csv_last_seen"] = base_df["Time"].iloc[-1]
            ingested_real = True

    # Always produce a synthetic tick when no real data arrived
...
def page_realtime(df):
    # -------------------------------------------------------
    # FIX 3: Use st_autorefresh instead of time.sleep+st.rerun
    # The checkbox controls whether the timer is active.
    # st_autorefresh fires a full re-run from the browser every
    # 3 000 ms without blocking Streamlit's server thread.
    # -------------------------------------------------------
    col_title, col_status, col_refresh = st.columns([3, 2, 1])
    with col_refresh:
        auto_refresh = st.checkbox("Auto-refresh", value=False)

    # Place the autorefresh call BEFORE rendering any data so the
    # very first re-run already has fresh values in the buffer.
    if auto_refresh:
    ...
    # NOTE: time.sleep + st.rerun removed entirely — st_autorefresh above handles this cleanly.
...
def main():
    with st.sidebar:
        ...
        # FIX 1: provide a non-empty label and hide it with label_visibility
        page = st.radio(
            label="Navigation",
